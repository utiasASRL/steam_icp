...
bool SteamLioOdometry::icp(int index_frame, std::vector<Point3D> &keypoints, const std::vector<IMUData> &imu_data_vec,
  ...
  std::vector<BaseCostTerm::ConstPtr> T_sr_prior_cost_terms;
  ...
  lgmath::se3::Transformation prev_T_sr = trajectory_vars_.back().T_sr->value();
  ...
  auto prev_T_sr_var = trajectory_vars_.back().T_sr;

  if (options_.estimate_T_sr) {
    steam_state_vars.emplace_back(prev_T_sr_var);
  } else {
    prev_T_sr_var->locked() = true;
  }
  ...
  for (size_t i = 0; i < knot_times.size(); ++i) {
    ...
    const auto T_sr_var = SE3StateVar::MakeShared(prev_T_sr);
    if (!options_.estimate_T_sr) {
      T_sr_var->locked() = true;
    } else {
      steam_state_vars.emplace_back(T_sr_var);
    }
  ...
  if (index_frame == 1) {
    ...
    P0_inv_ = Eigen::Matrix<double, 18, 18>::Zero();
    P0_inv_.block<6, 6>(0, 0) = P0_pose.inverse();
    P0_inv_.block<6, 6>(6, 6) = P0_vel.inverse();
    P0_inv_.block<6, 6>(12, 12) = P0_accel.inverse();
  ...
  if (options_.use_imu) {
    if (index_frame == 1) {
      ...
      const auto P = P0_inv_;
      P0_inv_.resize(24, 24);
      P0_inv_.setZero();
      P0_inv_.block<18, 18>(0, 0) = P;
      P0_inv_.block<6, 6>(18, 18) = init_bias_cov.inverse();
    ...
    if (index_frame == 1 && !use_T_mi_gt) {
      ...
      const auto P = P0_inv_;
      P0_inv_.resize(30, 30);
      P0_inv_.setZero();
      P0_inv_.block<24, 24>(0, 0) = P;
      P0_inv_.block<6, 6>(24, 24) = init_T_mi_cov.inverse();
    }
  ...
  if (index_frame == 1 && options_.estimate_T_sr) {
    const auto &prev_var = trajectory_vars_.at(prev_trajectory_var_index);
    Eigen::Matrix<double, 6, 6> init_T_sr_cov = Eigen::Matrix<double, 6, 6>::Zero();
    init_T_sr_cov.diagonal() = options_.T_sr_init_cov;
    lgmath::se3::Transformation T_sr = prev_var.T_sr->value();
    auto T_sr_error = se3_error(prev_var.T_sr, T_sr);
    auto noise_model = StaticNoiseModel<6>::MakeShared(init_T_sr_cov);
    auto loss_func = L2LossFunc::MakeShared();
    const auto T_sr_prior_factor = WeightedLeastSqCostTerm<6>::MakeShared(T_sr_error, noise_model, loss_func);
    T_sr_prior_cost_terms.emplace_back(T_sr_prior_factor);
  }
    ...
    if ((index_frame - options_.delay_adding_points) > 0) {
      ...
      for (size_t i = to_marginalize_; i <= curr_trajectory_var_index; ++i) {
        ...
        if (var.time <= marg_steam_time) {
          ...
          if (!var.T_sr->locked()) marg_vars.emplace_back(var.T_sr);
          num_states++;
        } else {
          to_marginalize_ = i;
          break;
      ...
      // sliding_window_filter_->marginalizeVariable(marg_vars, P0_inv_, Qk_inv_);
  ...
  std::map<double, Evaluable<const_vel::Interface::PoseType>::ConstPtr> T_ms_intp_eval_map;
  // std::vector<Evaluable<const_vel::Interface::PoseType>::ConstPtr> T_sm_intp_eval_vec;
  // bool use_T_ms_p2p = true;

  std::set<double> unique_point_times;
  for (const auto &keypoint : keypoints) {
    unique_point_times.insert(keypoint.timestamp);
  }
  for (const auto &ts : unique_point_times) {
    ...
    T_ms_intp_eval_map[ts] = T_ms_intp_eval;
  }
  ...
  if (options_.use_imu) {
    ...
    // Get IMU prior cost terms
    {
      ...
      const auto Q = Qk_inv_;
      Qk_inv_.resize(24, 24);
      Qk_inv_.setZero();
      Qk_inv_.block<18, 18>(0, 0) = Q;
      Qk_inv_.block<6, 6>(18, 18) = bias_cov.inverse();
    }
    ...
    if (!options_.T_mi_init_only && !use_T_mi_gt) {
      ...
      const auto Q = Qk_inv_;
      Qk_inv_.resize(30, 30);
      Qk_inv_.setZero();
      Qk_inv_.block<24, 24>(0, 0) = Q;
      Qk_inv_.block<6, 6>(24, 24) = T_mi_cov.inverse();
    }
  }

  // T_sr prior cost terms
  {
    if (options_.estimate_T_sr) {
      const auto T_sr = lgmath::se3::Transformation();
      Eigen::Matrix<double, 6, 6> T_sr_cov = Eigen::Matrix<double, 6, 6>::Zero();
      T_sr_cov.diagonal() = options_.q_T_sr_diag;
      auto noise_model = StaticNoiseModel<6>::MakeShared(T_sr_cov);
      auto loss_func = L2LossFunc::MakeShared();
      size_t i = prev_trajectory_var_index;
      for (; i < trajectory_vars_.size() - 1; i++) {
        auto T_sr_error = se3_error(compose_rinv(trajectory_vars_[i + 1].T_sr, trajectory_vars_[i].T_sr), T_sr);
        const auto T_sr_prior_factor = WeightedLeastSqCostTerm<6>::MakeShared(T_sr_error, noise_model, loss_func);
        T_sr_prior_cost_terms.emplace_back(T_sr_prior_factor);
      }
    }
...
#pragma omp parallel for num_threads(options_.num_threads)
    for (const auto &ts : unique_point_times) {
      const auto &T_ms_intp_eval = T_ms_intp_eval_map[ts];
      const Eigen::Matrix4d T_ms = T_ms_intp_eval->value().matrix();
#pragma omp critical { T_ms_cache_map[ts] = T_ms; }
    }
#pragma omp parallel for num_threads(options_.num_threads)
...
#pragma omp parallel for num_threads(options_.num_threads) reduction(merge_meas : meas_cost_terms)
    for (int i = 0; i < (int)keypoints.size(); i++) {
      ...
      if (use_p2p) {
        ...
        const auto &T_ms_intp_eval = T_ms_intp_eval_map[keypoint.timestamp];
    ...
    for (const auto &cost : T_sr_prior_cost_terms) problem.addCostTerm(cost);
    ...
    std::cout << "w(-1): " << trajectory_vars_[trajectory_vars_.size() - 1].w_mr_inr->value().transpose() << std::endl;
    std::cout << "w(-2): " << trajectory_vars_[trajectory_vars_.size() - 2].w_mr_inr->value().transpose() << std::endl;
    std::cout << "dw(-1): " << trajectory_vars_[trajectory_vars_.size() - 1].dw_mr_inr->value().transpose()
              << std::endl;
  ...
  for (const auto &T_sr_prior_cost : T_sr_prior_cost_terms) sliding_window_filter_->addCostTerm(T_sr_prior_cost);
